...
enum MobileAdvancedAuthenticationError: LocalizedError {
...
@MainActor
final class MobileAdvancedAuthenticationStore: ObservableObject {
    static let shared = MobileAdvancedAuthenticationStore()
    ...
    func delete(_ identity: AdvancedAuthIdentityRecord) {
        do {
            if identity.kind == .secureEnclaveKey, let account = identity.keychainAccount {
                try MobileSecureEnclaveSSHIdentityStore.shared.deleteKeyReference(account: account)
            }
            var data = try integrationStore.load()
...
final class MobileSecureEnclaveSSHIdentityStore {
    ...
    private let challenge = Data("agent-ssh-mobile-secure-enclave-probe".utf8)

    private init() {}

    func generateIdentity(label rawLabel: String) throws -> AdvancedAuthIdentityRecord {
        guard SecureEnclave.isAvailable else { throw MobileAdvancedAuthenticationError.secureEnclaveUnavailable }
        ...
        guard let accessControl = SecAccessControlCreateWithFlags(
            ...
            throw MobileAdvancedAuthenticationError.secureEnclaveUnavailable
        }
        ...
        let publicKey = Self.openSSHPublicKey(
            ...
            comment: "agent-ssh-ios-\(id.prefix(8))"
        )
        ...
        return AdvancedAuthIdentityRecord(
            ...
            publicKey: publicKey,
            publicKeyFingerprint: MobileSSHKeyVault.fingerprint(publicKeyLine: publicKey),
            keychainAccount: id,
            createdAt: Date(),
            updatedAt: Date(),
    ...
    func signProbe(identity: AdvancedAuthIdentityRecord) throws -> String {
        guard identity.kind == .secureEnclaveKey,
            ...
            throw MobileAdvancedAuthenticationError.identityNotFound
        }
        let context = LAContext()
    ...
    func importSSHCertificate(from url: URL) throws -> AdvancedAuthIdentityRecord {
        ...
        guard line.contains("-cert-v01@openssh.com") else {
            throw MobileAdvancedAuthenticationError.invalidCertificate
        }
        ...
        return AdvancedAuthIdentityRecord(
            ...
            publicKey: line,
            publicKeyFingerprint: MobileSSHKeyVault.fingerprint(publicKeyLine: line),
            keychainAccount: nil,
            certificate: line,
            createdAt: Date(),
            updatedAt: Date(),
    ...
    func importSecurityKeyPublicKey(from url: URL) throws -> AdvancedAuthIdentityRecord {
        ...
        guard type.hasPrefix("sk-") || type.contains("-sk-") else {
            throw MobileAdvancedAuthenticationError.invalidSecurityKey
        }
        let comment = Self.comment(from: line) ?? url.deletingPathExtension().lastPathComponent
        return AdvancedAuthIdentityRecord(
            kind: .securityKey,
    ...
    func loadKeyReference(account: String) throws -> Data {
        ...
        guard status == errSecSuccess, let data = item as? Data else {
            throw status == errSecItemNotFound
                ? MobileAdvancedAuthenticationError.identityNotFound
                : MobileAdvancedAuthenticationError.keychainStatus(status)
        }
        return data
    }
    ...
    func deleteKeyReference(account: String) throws {
        ...
        guard status == errSecSuccess || status == errSecItemNotFound else {
            throw MobileAdvancedAuthenticationError.keychainStatus(status)
        }
    }
    ...
    private static func openSSHPublicKey(x963Representation: Data, comment: String) -> String {
        ...
        var blob = Data()
        blob.appendMobileAdvancedAuthSSHString(Data(keyType.utf8))
        blob.appendMobileAdvancedAuthSSHString(Data("nistp256".utf8))
        blob.appendMobileAdvancedAuthSSHString(x963Representation)
        return "\(keyType) \(blob.base64EncodedString()) \(comment)"
    }
}

private extension Data {
    mutating func appendMobileAdvancedAuthUInt32(_ value: UInt32) {
        var bigEndian = value.bigEndian
        append(Data(bytes: &bigEndian, count: MemoryLayout<UInt32>.size))
    }

    mutating func appendMobileAdvancedAuthSSHString(_ data: Data) {
        appendMobileAdvancedAuthUInt32(UInt32(data.count))
        append(data)
    }
}
